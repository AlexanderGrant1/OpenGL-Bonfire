...
I modified this to give a terrain that suits my scene
*/
...
terrain_object::terrain_object(GLuint modelID,GLuint textureID, GLuint forestBumpTexture)
{
	attribute_v_coord = 0;
	attribute_v_normal = 1;
	attribute_tex_coord = 2;
	xsize = 0;	// Set to zero because we havent' created the heightfield array yet
	zsize = 0;	
	perlin_octaves = 4;
	height_scale = 1.f;
	texture_id = textureID;
	bump_texture = forestBumpTexture;
	model_id = modelID;
}
...
//Calculate texture coordinates for the terrain
void terrain_object::calculateTextureCoordinates()
{
	for (int row = 0; row < zsize; row++)
	{
		for (int col = 0; col < xsize; col++)
		{
			float x = (float)row / zsize;
			float y = (float)col / xsize;
			y = 1 - y;
			x *= 10;
			y *= 10;//repeat the texture 10 times
...
void terrain_object::createObject()
{
	...
	glBufferData(GL_ARRAY_BUFFER, xsize * zsize * sizeof(glm::vec2), &(tex_coords[0]), GL_STATIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
...
void terrain_object::drawObject(int drawmode)
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, vbo_tex_coords);
	glVertexAttribPointer(
		attribute_tex_coord,  // attribute index
